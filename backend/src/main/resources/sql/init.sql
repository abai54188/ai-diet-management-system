...
CREATE TABLE IF NOT EXISTS diet_record (
    ...
    `user_id`     BIGINT       NOT NULL COMMENT '用户ID',
    `food_id`     BIGINT       DEFAULT NULL COMMENT '食材ID(可空: AI整道菜打卡无单一食材)',
    `food_name`   VARCHAR(50)  NOT NULL DEFAULT '' COMMENT '食材名称(冗余存储; AI整道菜打卡存菜名)',
    `weight`      DECIMAL(6,1) NOT NULL COMMENT '食用重量(g)',
    `calorie`     DECIMAL(8,1) NOT NULL COMMENT '摄入热量(kcal, 按 weight 计算后落库)',
    `protein`     DECIMAL(10,1) DEFAULT NULL COMMENT '蛋白质(g, AI整菜打卡直接落库)',
    `carbohydrate` DECIMAL(10,1) DEFAULT NULL COMMENT '碳水(g, AI整菜打卡直接落库)',
    `fat`         DECIMAL(10,1) DEFAULT NULL COMMENT '脂肪(g, AI整菜打卡直接落库)',
