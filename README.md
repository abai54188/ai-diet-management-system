...
> 前端采用 Vue 3 + Vite，后端采用 Spring Boot + SQLite，前后端可**一体化打包成单个 jar**，双击脚本即可启动，无需单独部署前端或安装数据库。页面支持 PC 与手机浏览器**响应式自适应**访问。

> 🌐 **在线展示**：<https://abai54188.github.io/ai-diet-management-system/>（GitHub Pages 演示模式，免登录浏览界面 UI；数据交互需本地启动后端）
...
| 模块 | 说明 |
| --- | --- |
| 🥗 营养与食谱 | 查询 1200+ 食材营养数据，多选食材构建食谱并实时计算热量/蛋白质/碳水/脂肪 |
| 🤖 AI 食谱生成 | 根据已选食材 + 健康档案，调用大模型生成多套推荐菜单与详细做法 |
| 📷 拍照识菜 | 拍摄/上传食物照片，AI 视觉模型识别菜品、估算食材重量并计算热量等营养 |
| 🧠 营养助手 | 大模型对话式营养咨询 |
...
│       ├── data.sql              # 种子数据(admin 账号 + 1203 条食材)
│       └── application.yml       # 主配置
├── docs/                         # 文档
│   ├── 项目说明书.md
│   ├── 项目使用说明.md
...
> 💡 **非开发者想直接使用？** 只需安装 Java 17，然后到 [Releases](https://github.com/abai54188/ai-diet-management-system/releases) 下载 `diet-backend-1.0.0.jar`，配合仓库根目录的 `start.bat` 双击运行即可。完整图文步骤见 [docs/非开发者运行指南.md](docs/非开发者运行指南.md)。
...
- [非开发者运行指南](docs/非开发者运行指南.md)：无需编程，安装 Java 后双击运行（推荐普通使用者阅读）
